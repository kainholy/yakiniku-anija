...
const data = [
  {
    ...
    title:
      '当店では最高な空間と共に味わえる和牛、職人の工夫が最大限活かされた料理をお客様に届けていきます。カット方法、味付けからこだわったお肉たちをご堪能下さい。',
  ...
  },
  {
    ...
    title:
      '肉が上手いだけじゃない！ドリンクもリーズナブルに来ていただいたお客様が満足してもらえる、そんな兄者ドリンクをお肉と一緒に楽しんでください。',
  ...
  },
  {
    ...
    title:
      '「和」の落ち着いた伝統的な雰囲気と、「楽」の明るい活気のある空間。二つのコンセプトから創造する唯一無二の食体験をご用意しております。お客様の好みや気分に合わせた刻の一滴をご堪能下さい。',
        ...
        <div className={base + '__content-area'}>
          {data.map((item, index) => (
            <div key={index} className={base + '__item ' + item.className}>
              <div className={base + '__item__bg'}></div>
              <div className={base + '__item__text-area'}>
                <p
                  className={
                    base + '__item__number wh c-top-strong-point-number'
                  }
                >
                  {item.number}
                </p>
                ...
                <p className={base + '__item__text wh c-top-strong-point-text'}>
                  {item.title}
                </p>
              </div>
